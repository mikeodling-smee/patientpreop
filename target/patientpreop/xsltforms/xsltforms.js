...
if (window.XMLHttpRequest) {
	Core.openRequest = function(method, uri, async) {
		...
		//alert("Opening request - method: " + method + ", Uri: " + uri + ", Async? " + async);
...
XFSubmission.prototype.submit = function() {
	...
	if(method == "xml-urlencoded-post") {
		...
		if (action.match(/^[a-zA-Z0-9+.-]+:\/\//)) {
			...
			var ldom = domain.exec(document.location.href);
			cross = sdom[0] != ldom[0];
			//alert("Cross: " + cross + ", sdom: " + sdom + ", ldom: " + ldom);
		}
		if (cross) {
			//alert("Here 2");
			...
			var subm = this;
			try {
				//alert("Submission method: " + method + ",  resource: " + action + ", ASync?: " + !synchr);
				...
				var func = function() {
					...
					try {
						//alert("b");
						if (req.status != 200 && req.status != 202 && req.status != 0) {
